...
static void xclearbg(Color *, int, int, int, int);
 ...
 */
void
xclearbg(Color *color, int x1, int y1, int x2, int y2)
{
	XftDrawRect(xw.draw, color, x1, y1, x2-x1, y2-y1);
}
...
xinit(int cols, int rows)
{
	...
	xw.attrs.border_pixel = dc.col[bordercolor].pixel;
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int dmode)
{
	...
	if (dmode & DRAW_FG) {
		...
		r.y = y;
		r.height = win.ch;
		r.width = width + win.cw; /* + win.cw to support wide nerd fonts */
		...
		if (base.mode & ATTR_UNDERLINE) {
			XftDrawRect(xw.draw, fg, x, y + dc.font.ascent * chscale + 1, width, 1);
		}

		if (base.mode & ATTR_STRUCK) {
			XftDrawRect(xw.draw, fg, x, y + 2 * dc.font.ascent * chscale / 3, width, 1);
		}
